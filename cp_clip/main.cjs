...
// -------------------------------------------------------------------------------
// YouTube Cookie & Login Sync Manager
// -------------------------------------------------------------------------------
function getYtCookieConfigFile() {
  return path.join(app.getPath('userData'), 'yt_cookie_config.json');
}

function getYtCookiesFilePath() {
  return path.join(app.getPath('userData'), 'youtube_cookies.txt');
}

function getYtCookieConfig() {
  const cfgPath = getYtCookieConfigFile();
  let config = { mode: 'none' };
  if (fs.existsSync(cfgPath)) {
    try {
      config = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
    } catch (e) {
      console.warn('[YT-COOKIE] Failed to read cookie config:', e.message);
    }
  }
  const cookiesFile = getYtCookiesFilePath();
  const hasEmbeddedCookies = fs.existsSync(cookiesFile) && fs.statSync(cookiesFile).size > 100;
  return {
    mode: config.mode || 'none', // 'none' | 'edge' | 'chrome' | 'firefox' | 'brave' | 'embedded'
    hasEmbeddedCookies,
    cookiesPath: hasEmbeddedCookies ? cookiesFile : null
  };
}

function saveYtCookieConfig(cfg) {
  try {
    fs.writeFileSync(getYtCookieConfigFile(), JSON.stringify(cfg, null, 2), 'utf8');
  } catch (e) {
    console.error('[YT-COOKIE] Failed to save cookie config:', e.message);
  }
}

function exportCookiesToNetscape(cookies) {
  const lines = [
    '# Netscape HTTP Cookie File',
    '# http://curl.haxx.se/rfc/cookie_spec.html',
    '# This file was generated by ShareCLIP. Do not edit!',
    ''
  ];
  (cookies || []).forEach(c => {
    if (!c.name || !c.value) return;
    const domain = c.domain ? (c.domain.startsWith('.') ? c.domain : ('.' + c.domain)) : '.youtube.com';
    const includeSubdomains = 'TRUE';
    const cookiePath = c.path || '/';
    const isSecure = c.secure ? 'TRUE' : 'FALSE';
    const expires = c.expirationDate ? Math.round(c.expirationDate) : (Math.round(Date.now() / 1000) + 86400 * 365);
    lines.push(`${domain}\t${includeSubdomains}\t${cookiePath}\t${isSecure}\t${expires}\t${c.name}\t${c.value}`);
  });
  return lines.join('\n') + '\n';
}
...
ipcMain.handle('yt-cookies-get-config', async () => {
...
ipcMain.handle('yt-open-login-window', async (event) => {
  ...
  ytLoginWindow.setMenuBarVisibility(false);

  const targetUrl = 'https://accounts.google.com/ServiceLogin?service=youtube&continue=https%3A%2F%2Fwww.youtube.com%2F';
  ytLoginWindow.loadURL(targetUrl, { userAgent: desktopUserAgent });
  ...
  const extractAndSaveCookies = async () => {
    ...
    try {
      ...
      const hasAuth = cookies.some(c => ['LOGIN_INFO', 'SID', 'HSID', 'SSID'].includes(c.name));
      if (hasAuth) {
        captured = true;
        if (checkInterval) clearInterval(checkInterval);

        const googleCookies = await loginSession.cookies.get({ domain: '.google.com' });
  ...
  checkInterval = setInterval(extractAndSaveCookies, 1500);

  ytLoginWindow.webContents.on('did-navigate', async (e, navUrl) => {
    if (navUrl.includes('youtube.com') && !navUrl.includes('ServiceLogin')) {
...
ipcMain.handle('yt-cookies-clear', async () => {
  try {
    const cookieFile = getYtCookiesFilePath();
    ...
    return new Promise((resolve) => {
      ...
      child.on('close', code => {
        if (code === 0) {
          ...
          if (errText.includes('database is locked') || errText.includes('Could not copy') || errText.includes('Could not decrypt')) {
            errText += '\n💡 提示：检测到本地浏览器 Cookie 数据库被占用或锁表。建议关闭对应浏览器，或在右上角【🔐 登录同步】切换为【内嵌一键登录】。';
          }
          console.error('[YT-DLP ERR]', errText);
