...
*   [Low-End CPU & 4GB RAM AI Inference Optimization](file:///d:/AI_serach_image/image_clip_android/wiki/features/low_end_cpu_ai_optimization.md): 3-tier ONNX provider fallback (CPU AVX2 -> DirectML GPU -> Safe CPU) fixing non-AVX2 chips (Celeron N4020/N4120), on-demand lazy model loading saving >140MB RAM, zero-allocation buffer pooling, and strict hardware tier isolation.
...
*   [Differential Update & Cache Purging Architecture](file:///d:/AI_serach_image/image_clip_android/wiki/features/differential_update_and_cache_purging.md): Complete forensic analysis of differential update failure and fallback to full 148MB download, cache desync root causes, `sanitizeUpdaterCache()` automated cache purging, 60s watchdog relaxation, and saving 94.87% update bandwidth (7.64MB vs 148MB).
*   [Video Downloader Post-processing, Portable FFmpeg & Code Signing Architecture](file:///d:/AI_serach_image/image_clip_android/wiki/features/code_signing_and_ffmpeg_resilience.md): In-depth root cause analysis of yt-dlp 100% exit code 1 errors due to missing FFmpeg on clean machines, `ensureFFmpeg` portable auto-provisioning, graceful single-stream fallback, and dual-platform free public code signing (Windows RFC 3161 Authenticode self-signed + 1-click installer, macOS Ad-hoc + Gatekeeper bypass, and Linux Foundation Sigstore Rekor Artifact Attestations).
...
*   [⚡ 双核/低配电脑与 4GB 极低内存 AI 推理引擎极致优化、多层容灾与全机型分级 (Low-End CPU & 4GB RAM AI Inference Optimization)](file:///d:/AI_serach_image/image_clip_android/wiki/features/low_end_cpu_ai_optimization.md): 针对 81.6% 存量双核/四核超极本与 4GB 内存老旧机型，构建三级 ONNX 容灾降级（CPU AVX2 ➔ DirectML GPU ➔ Safe CPU）根除赛扬/无 AVX2 设备崩退；人脸/文本模型按需延迟加载释放 >140MB 内存；WorkerPool 异常防挂死；结合静态零拷贝张量池与缩略图快速流，6000 张相片全量预测提速 3 倍且高低配互不影响。 **Status: ✅ Implemented**
...
*   [🎬 多站点 4K 视频解析下载器、独立 HTML5 Electron 播放器窗口与高对比度界面体系 (4K Multi-Site Video Downloader & Detached Electron Player)](file:///d:/AI_serach_image/image_clip_android/wiki/features/yt_dlp_video_downloader_and_player.md): 深度集成 yt-dlp 4K 极速解析下载内核，支持 YouTube、Bilibili、抖音、快手、Twitter/X、小红书等 1000+ 平台；智能剪贴板文案提取清洗；FFmpeg DASH 音视频自动无损混流封装与高清封面内嵌；自研独立无边框 HTML5 Video 播放器弹窗（GPU 硬件加速、Range 206 局部流式缓冲、画中画置顶 Pin 与全键盘控制）；以及 v4.0.0 浅色/深色模式 WCAG 2.1 AA 级高对比度界面体系。 **Status: ✅ Implemented**
*   [📢 全球化全自动推广、开源赞助体系与极客包管理器分发飞轮 (Automated Promotion & Growth Flywheel)](file:///d:/AI_serach_image/image_clip_android/wiki/features/automated_promotion_and_growth_engine.md): 落地 5 大增长引擎——GitHub 官方 Sponsors / Buy Me a Coffee / Patreon 赞助变现体系；WebShare 纯网页端内置自裂变引流挂件（PLG）；GitHub Actions CI/CD 自动发布宣发流水线（自动生成发布 Markdown 与 Discord/Telegram 社区广播）；Windows 极客包管理器自动感知（Scoop & WinGet 清单与 SHA256 计算工具）；以及全套顶级公域社区（AlternativeTo、Reddit、Hacker News、Product Hunt、Awesome Lists）打榜物料与程序化 SEO（sitemap.xml / robots.txt）。 **Status: ✅ Implemented**
*   [⚡ Electron 差分增量升级深度排查、本地缓存净化与防降级加固架构 (Differential Update & Cache Purging Architecture)](file:///d:/AI_serach_image/image_clip_android/wiki/features/differential_update_and_cache_purging.md): 深度还原差分升级报错（SHA512 不匹配）并被迫回退全量 148MB 下载的四大根因，实现 `sanitizeUpdaterCache()` 启动自愈净化、60s 本地盘内搬运弹性防误杀看门狗、以及单分块串行下载规避 Azure 501 报错，升级包体积从 148.78MB 锐减至 7.64MB，节省 94.87% 带宽。 **Status: ✅ Implemented**
*   [🔏 视频下载后处理排查、便携 FFmpeg 自愈与 GitHub 免费公有代码签名体系 (Video Downloader Resilience & Free Public Code Signing)](file:///d:/AI_serach_image/image_clip_android/wiki/features/code_signing_and_ffmpeg_resilience.md): 彻底解决普通电脑无 FFmpeg 导致 yt-dlp 下载 100% 暴毙 exit code 1 问题，支持本地沙箱便携 FFmpeg 静默按需拉取与单流优雅降级；构建 GitHub Actions Win/Mac 双平台免费公有签名体系（Windows Authenticode 时间戳自签名与一键信任脚本、macOS Ad-hoc 签名与 Sequoia Gatekeeper 一键修复脚本、以及 Sigstore Rekor 全球公共透明存证）。 **Status: ✅ Implemented**
...
|---|---|---|
| **v4.0.1** | 2026-09-11 | **4K 视频下载后处理增强、便携版 FFmpeg 自动静默拉取、Windows/macOS 免费公有签名与全球透明存证**：<br>① **便携版 FFmpeg 自动静默拉取与自愈**：针对无环境 PATH 的纯净机型，新增 `ensureFFmpeg()` 后台探针，静默拉取免安装 FFmpeg 至 `userData/bin`，并在 `resolveFFmpegPaths()` 优先加载；<br>② **极端无混流器场景动态优雅降级**：在 FFmpeg 就绪前，`yt-download` 自适应降级为预混单流格式（`best[ext=mp4]/best`），关闭需要转码的后处理选项，原画海报直接落盘，彻底根除 100% 进度时因缺失 FFmpeg 导致的 `exit code 1` 退出错误；<br>③ **Windows RFC 3161 Authenticode 证书自签与一键信任**：流水线动态生成代码签名证书，调用 `signtool.exe` 结合 DigiCert 权威时间戳签名，打包随附 `Install-Certificate.bat` 一键将证书注入根信任库，彻底消除 SmartScreen 未知发布者红框阻拦；<br>④ **macOS Ad-hoc 深度签名与 Gatekeeper 一键修复**：打包配置启用 `mac.identity: "-"` 并递归执行 `codesign --force --deep -s -`，配套输出 `Fix_App_Damage.command` 一键移除 Sequoia 15+ 隔离属性（`xattr -cr`），告别“文件已损坏”拦截；<br>⑤ **Linux Foundation Sigstore + Rekor 全球透明存证**：引入官方 `actions/attest-build-provenance@v2`，对全量 Release 资产签署不可篡改的加密 Provenance 存证；<br>⑥ **全生态版本同步至 v4.0.1**：PC 端、Vue 落地页、WebShare、Android（`4.0.1+40001`）与 Scoop/WinGet 清单全面对齐。 |
| **v4.0.0** | 2026-09-10 | **🎉 4.0 重大里程碑版本：界面视觉与文字高对比度全面重构、全端升级与发版**：<br>① **浅色模式（Light Mode）视觉与对比度彻底重构**：根除暗夜行内硬编码样式在浅色底色下的脏灰污斑与深灰底色冲突，补齐 `--primary` 别名变量（深色 `#6366f1` / 浅色 `#4f46e5`），全面引入 `.yt-segmented-tabs`、`.yt-tab-btn`、`.yt-input-card`、`.yt-card`、`.btn-danger-subtle` 等自适应组件体系，严格满足 WCAG 2.1 AA 级对比度规范；<br>② **全生态大版本升级至 v4.0.0**：全平台客户端（Electron 桌面端、Vue 落地页、WebShare 极速端、Android 客户端 `4.0.0+40000`、Scoop / WinGet 清单）统一升级至 4.0.0 并通过构建验证；<br>③ **双向发布与推送**：GitHub `main` 分支与 `v4.0.0` 标签已推送，Gitee `master` 分支与 `v4.0.0` 标签已同步推送。 |
| **v3.0.17** | 2026-09-10 | **多站点 4K 视频解析下载器、独立 HTML5 Electron 弹窗播放器与全自动推广飞轮落地**：<br>① **独立 HTML5 Electron 弹窗播放器**：彻底告别本地外部播放器慢启动，基于轻量独立窗口实现毫秒级拉起、无边框极客交互、始终置顶 Pin 浮窗、HTTP Range 206 局部流式解码与全键盘快捷键；<br>② **多站点 4K 视频解析下载引擎 (yt-dlp)**：支持 YouTube、Bilibili、抖音、快手、Twitter/X、小红书等全球 1000+ 平台，内置智能剪贴板文案提取清洗正则，自动实现 DASH 音视频分离流下载与 FFmpeg 无损混流封装，全自动内嵌高清海报与元数据；<br>③ **全球化全自动推广与变现体系落地**：集成 GitHub Sponsors / Buy Me a Coffee / Patreon 赞助入口、WebShare 产品自裂变引流挂件（PLG）、CI/CD 自动化多渠道发布宣发流（`.github/workflows/auto-promote.yml`）、Scoop / WinGet 极客包管理器自动清单、全套海外公域社区打榜物料及 sitemap/robots 搜索引擎索引。 |
| **v3.0.16** | 2026-09-08 | **低端机扫码极速优化、二维码颗粒度极致精简、局域网物理真实 IP 智能过滤与全端同步**：<br>① **低端手机扫码颗粒度大幅优化（秒扫识别）**：精简二维码载荷，剔除 72 字节的静态 Nordic UART UUID 常量（由移动端内置默认协议解析），缩短 JSON 键名（`ble_mac`➔`m`, `session_id`➔`s`, `pc_ips`➔`ip`），载荷由 240+ 字节锐减 75% 至 ~60 字符；设置 `errorCorrectionLevel: 'L'`（低容错），二维码矩阵由高密度的 53x53 (2,809 点) 骤降至 25x25 (625 点) 稀疏大颗粒，画布放大至 160px，码点面积增大超 400%，老旧/低端机摄像头无需精密对焦即可毫秒级瞬间解码；<br>② **局域网物理真实 IP 多阶智能过滤**：PC 端集成内核级 UDP 路由探针 + Windows 路由表 Metric 权重探测 + 虚拟网卡多层黑名单过滤（全面排除 VMware / VirtualBox / WSL / Hyper-V / Docker / VPN 等虚拟适配器与子网），保证二维码携带的 IP 100% 为局域网真实通信地址；手机端增加虚拟保留 IP 二次过滤防护并支持局域网极速并发直连；<br>③ **WebRTC 连接状态机与信令时序强化**：PC 端复用处理中的连接 Promise，杜绝 Offer/Answer 并发覆盖，消除 PC 端已就绪而手机端仍转圈等待的连接不同步缺陷；<br>④ **全平台版本同步递增至 v3.0.16**。 |
| **v3.0.15** | 2026-09-07 | **macOS 双架构原生编译与兼容性彻底修复、CI/CD 双机型流水线与全平台版本同步**：<br>① **修复 Intel Mac 启动 `node_sqlite3.node` 架构不匹配崩溃**：针对 GitHub Actions `macos-latest` 迁移为 Apple Silicon (arm64) 导致的 x64 安装包混入 arm64 原生模块的致命缺陷，将 GitHub Actions 流水线重构为**双机型矩阵并行原生构建（Matrix Native Runners）**——Apple Silicon 版在 `macos-latest` (arm64) 原生打包，Intel 版在 `macos-13` (x86_64) 原生打包，彻底消除架构混淆，生成的 `.dmg` 与 `.zip` 100% 适配对应芯片；<br>② **优化 `electron-builder` 构建配置**：重构 `mac.target` 为解耦形式，确保 CLI 架构参数精准控制，防止 cross-arch 缓存污染；<br>③ **Pages 部署与 Tag 触发健壮性修复**：修复 Pages 部署工作流中 token 参数语法规范，增强发布流水线 Tag 触发机制与版本号自动推断能力；<br>④ **全平台版本同步递增至 v3.0.15**。 |
| **v3.0.14** | 2026-09-04 | **4GB 内存低配电脑 AI 计算全链路加固、多层推理容灾回退与按需延迟加载**：<br>① **多层 ONNX 推理容灾回退（CPU ➔ DirectML ➔ Safe CPU）**：针对 4GB 内存电脑中常见缺少 AVX2 向量指令集的赛扬（Celeron N4020/N4120/Pentium Silver）等低端 CPU，构建三级降级链——优先 CPU AVX2 高速模式，若指令集不支持抛错立即自动回退至全系核显标配的 **DirectML (GPU)** 模式，最后保底单线程安全 CPU 模式，彻底根除 4GB 低配设备无法运行计算的痛点；<br>② **WorkerPool 进程崩溃与退出防悬挂**：工作线程无论因内存超限退出还是初始化失败，均即时 reject 等待的任务并触发队列补派，杜绝前端 Promise 永久死锁在“计算中...”；<br>③ **人脸/文本模型按需延迟加载 (Lazy Loading)**：相册批量 AI 分类期间不加载 SCRFD 人脸检测与 MobileFaceNet 模型，仅在用户触发人脸聚类时按需异步加载；主进程文本编码器仅在用户文字搜图时加载，**启动与批量分类阶段为 4GB 设备释放 >140MB 物理内存**；<br>④ **硬件分级边界精细化**：精准划分 Low（$\le 4.5$GB 内存，单 Worker + 2 线程）、Mid（$4.5\text{GB} \sim 15\text{GB}$，双 Worker + 2 线程）与 High（$>15$GB，双 Worker + 4 线程），高低配机器各司其职，互不影响；<br>⑤ **自适应小批次 DB 刷新**：Low Tier 设备单次 DB 事务刷新降至 10 张，内存开销更小且实时进度更平滑。 |
