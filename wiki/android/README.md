...
*   [lib/views/qr_scanner_view.dart](file:///d:/AI_serach_image/image_clip_android/android/lib/views/qr_scanner_view.dart): Redesigned Holographic Cyber QR Scanner View. Features frosted glass floating app bar with back navigation, real-time flashlight (torch) toggle, front/rear camera switcher, and album QR image picker with `analyzeImage()`. Renders animated holographic laser sweep beam, precision rounded gradient corner brackets, subtle viewfinder breathing glow, and a bottom floating card for instant one-tap LAN PC connection. All labels and hints are fully localized via `LocalizationService`.
