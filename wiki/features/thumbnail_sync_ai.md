...
**互斥锁无法释放 Bug (isThumbnailSyncing 永久死锁)**：
- **触发场景**：当 PC 端触发“同步全部图片到 AI”时，如果手机端发现部分/全部缩略图在 PC 端**已存在（已缓存）**，手机端会跳过发送这些缩略图，导致 PC 端收不到这些图片的“完成事件”。PC 端的 `thumbSyncDone` 计数器永远无法追平 `thumbSyncTotal`，互斥锁 `isThumbnailSyncing` 永远保持 `true`，使得同步完成后按钮依然呈灰色不可点击状态。
- **解决方案 (双端哨兵机制)**：
