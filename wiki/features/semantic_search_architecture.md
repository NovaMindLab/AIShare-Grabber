...
### 2. 硬件级 SIMD 并行计算
系统将向量相似度计算交由独立的计算 Worker 执行，并充分利用 CPU 的 SIMD（Single Instruction Multiple Data）向量指令集（如 AVX2/AVX-512）。计算过程中直接从共享内存读取 512 维特征向量，通过向量化指令一次并行完成多个浮点数的乘加运算，大幅提升余弦相似度计算效率，使海量向量检索能够在毫秒级完成。
