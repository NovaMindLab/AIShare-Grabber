...
### 1.1 早期架构痛点
在早期版本中，我们尝试直接复用 **MobileCLIP 图像编码器** 的全图语义特征（或基于规则的中心裁剪）进行人物归类。然而实践中暴露出根本性缺陷：
- **语义混淆**：CLIP 是图文多模态“场景语义”模型，而非“人脸生物特征”模型。例如“穿着粉色衣服的真人”与“粉色头发的动漫手办”在 CLIP 512 维语义空间中极度接近，导致被错误归入同一人物。
- **多人合影失效**：一张照片中如果存在多个人物，全图 CLIP 向量无法解耦个体面容，导致合影照片无法被正确归纳到各个被拍者的独立相册中。

### 1.2 工业级级联方案 (Cascaded Pipeline)
从 **v1.2.54** 版本起，ShareCLIP 全面重构人脸架构，引入了 **SCRFD (人脸检测定位) ➔ 动态 ROI 几何对齐 ➔ MobileFaceNet (512-D ArcFace 度量学习) ➔ WASM SIMD 同图排他聚类** 的全套工业级离线解决方案。
