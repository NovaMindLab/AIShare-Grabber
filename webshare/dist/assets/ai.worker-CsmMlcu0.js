 ...
 * limitations under the License.
 * =============================================================================
 */let Et=null,lr=null,qr="unknown";ye.wasm.numThreads=1;ye.wasm.proxy=!1;const Bd=typeof self<"u"&&self.location&&self.location.origin?self.location.origin:"";ye.wasm.wasmPaths=Bd?`${Bd}/ort-wasm/`:"https://cdn.jsdelivr.net/npm/onnxruntime-web@1.18.0/dist/";async function F0(e){const t="webshare-ai-models-v2";let r;if("caches"in self)try{if(r=await(await caches.open(t)).match(e),r)return console.log(`[AI Worker] Loaded model from CacheStorage: ${e}`),await r.arrayBuffer()}catch(n){console.warn("[AI Worker] CacheStorage lookup failed:",n)}console.log(`[AI Worker] Fetching model from network: ${e}`);const i=await fetch(e);if(!i.ok)throw new Error(`Failed to fetch model from ${e}: ${i.status} ${i.statusText}`);const a=await i.arrayBuffer();if("caches"in self)try{await(await caches.open(t)).put(e,new Response(a.slice(0),{headers:{"Content-Type":"application/octet-stream"}})),console.log(`[AI Worker] Cached model into CacheStorage: ${e}`)}catch(n){console.warn("[AI Worker] CacheStorage write failed:",n)}return a}async function j0(e="/models/mobileclip2_s0_image_encoder.onnx",t="/models/text_embeddings.json"){try{console.log("[AI Worker] Loading text embeddings from:",t),lr=await(await fetch(t)).json(),console.log(`[AI Worker] Loaded ${Object.keys(lr).length} text category embeddings.`),console.log("[AI Worker] Fetching MobileCLIP2-S0 ONNX model buffer from:",e);const i=await F0(e);let a=!1;try{typeof navigator<"u"&&navigator.gpu&&await navigator.gpu.requestAdapter()&&(a=!0)}catch{}if(a)try{console.log("[AI Worker] Attempting ONNX Session with WebGPU provider..."),Et=await Lr.create(i,{executionProviders:["webgpu"]}),qr="webgpu",console.log("✨ [AI Worker] ONNX Session created successfully with WebGPU acceleration!")}catch(n){console.warn("[AI Worker] WebGPU session creation failed, falling back to WASM:",n.message)}Et||(console.log("[AI Worker] Creating ONNX Session with WASM CPU provider..."),Et=await Lr.create(i,{executionProviders:["wasm"]}),qr="wasm",console.log("⚡ [AI Worker] ONNX Session created successfully with WASM CPU provider.")),self.postMessage({type:"init_result",success:!0,provider:qr,categoryCount:Object.keys(lr).length})}catch(r){console.error("[AI Worker] Failed to initialize engine:",r),self.postMessage({type:"init_result",success:!1,error:r.message})}}async function K0(e){let t=e instanceof Blob?e:new Blob([e]);const r=await createImageBitmap(t),a=new OffscreenCanvas(256,256).getContext("2d",{willReadFrequently:!0}),n=Math.max(256/r.width,256/r.height),s=r.width*n,u=r.height*n,l=(256-s)/2,p=(256-u)/2;a.drawImage(r,l,p,s,u),r.close();const f=a.getImageData(0,0,256,256).data,h=new Float32Array(3*256*256),g=256*256;for(let y=0;y<g;y++)h[y]=f[y*4]/255,h[g+y]=f[y*4+1]/255,h[2*g+y]=f[y*4+2]/255;return h}function Z0(e,t){let r=0,i=0,a=0;for(let n=0;n<512;n++)r+=e[n]*t[n],i+=e[n]*e[n],a+=t[n]*t[n];return i===0||a===0?0:r/(Math.sqrt(i)*Math.sqrt(a))}function X0(e){if(!lr)return[];const t=[];for(const[s,u]of Object.entries(lr))if(u&&u.length>0){const l=Z0(e,u);t.push({category:s,score:l})}const r=60,i=t.map(s=>({category:s.category,exp:Math.exp(s.score*r)})),a=i.reduce((s,u)=>s+u.exp,0),n=i.map(s=>({category:s.category,score:a>0?s.exp/a:0}));return n.sort((s,u)=>u.score-s.score),n.slice(0,3)}async function Q0(e){const{jobId:t,photoId:r,buffer:i}=e,a=performance.now();try{if(!Et)throw new Error("AI Session not initialized");const n=performance.now(),s=await K0(i),u=performance.now()-n,l=Et.inputNames[0],p=new Ke("float32",s,[1,3,256,256]),f=performance.now(),h=await Et.run({[l]:p}),g=performance.now()-f,y=Et.outputNames[0],_=new Float32Array(h[y].data),w=X0(_),k=performance.now()-a;self.postMessage({type:"inference_result",jobId:t,photoId:r,success:!0,embedding:Array.from(_),categories:w,provider:qr,timings:{prep:Math.round(u),infer:Math.round(g),total:Math.round(k)}})}catch(n){console.error(`[AI Worker] Error processing photo ${r}:`,n),self.postMessage({type:"inference_result",jobId:t,photoId:r,success:!1,error:n.message})}}self.onmessage=async e=>{const t=e.data;t&&(t.type==="init"?await j0(t.modelUrl,t.textEmbUrl):t.type==="infer"&&await Q0(t))};
