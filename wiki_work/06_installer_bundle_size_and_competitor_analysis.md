...
```mermaid
flowchart TD
    subgraph ScoreMatrix ["主流相册竞品综合能力评级对比"]
        direction TB
        C1["<b>ShareCLIP (本项目)</b><br>• 安装便携度: ★★★★★ (~168MB 一键免配置)<br>• 离线可用性: ★★★★★ (100% 纯端侧零隐私泄露)<br>• 自然语言搜图: ★★★★★ (MobileCLIP2 毫秒级)<br>• 局域网直传: ★★★★★ (WebRTC P2P 30~50MB/s)<br>• 部署门槛: ★★★★★ (零运维双击即用)"]
        C2["<b>Immich (自托管 Docker)</b><br>• 安装便携度: ★☆☆☆☆ (>2.5GB 依赖庞大容器栈)<br>• 离线可用性: ★★★★★ (本地私有化)<br>• 部署门槛: ★★☆☆☆ (需配置 Docker/DB 门槛高)"]
        C3["<b>Google Photos (云端)</b><br>• 安装便携度: ★★★★★ (0MB 网页即用)<br>• 离线可用性: ☆☆☆☆☆ (必须强制全量上云)<br>• 局域网直传: ★☆☆☆☆ (强依赖公网带宽)"]
        C4["<b>Mylio Photos (商业版)</b><br>• 安装便携度: ★★★☆☆ (~420MB 偏重)<br>• 离线可用性: ★★★★☆ (本地同步)"]
    end
```
